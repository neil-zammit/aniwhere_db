...
// Remove Show Form Route
router.get('/removemyshows', ensureAuthenticated, (req, res) =>
  res.render('removemyshows', {
    layout: false
  })
);

// Delete Show from MyShows Route
router.post('/removemyshows', ensureAuthenticated, (req, res) => {
  // Retrieve data from form
  let { title, image } = req.body;
  // Retrieve current user
  const user_id = req.user;
